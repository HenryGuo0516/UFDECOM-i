#include "DEFS.h"
	...
	SUBROUTINE INFO_EXCH3D(VA,MASK,NAG_EVG,WT_EVG,NAG_IVG,WT_IVG)
...
c          IND = (/3,5,1,7/)
          ...
          DO J = 1,4
            VA_INT(J) = 
     * (VA(NAG_EVG(IND(J),I),K)*WT_EVG(IND(J),I)
     * *MASK4VAR(IND(J),I)
     * +VA(NAG_EVG(IND(J)+1,I),K)*WT_EVG(IND(J)+1,I)
     * *MASK4VAR(IND(J)+1,I))
     ...
     * ((MASK4VAR(IND(J),I)+MASK4VAR(IND(J)+1,I)).NE.0.) THEN
          ...
          !U/V > 0.
            ...
            IF ((MASK_INT(1)*MASK_INT(2)*MASK_INT(3)).NE.0.) THEN
            !HSIMT PARABOLIC INTERPOLATION
                
              !COEFFICIENTS
              COF1 = 
     * 0.5*(VA_INT(3)-VA_INT(2))-0.5*(VA_INT(2)-VA_INT(1))
              COF2 = VA_INT(3)-VA_INT(2)
              COF3 = 1./3.*(VA_INT(3)-VA_INT(2))
     * +1./6.*(VA_INT(2)-VA_INT(1))+VA_INT(2)
...
!                  CYCLE
     ...
     * /WEIALL
          ...
          !U/V<0.
            ...
            IF ((MASK_INT(2)*MASK_INT(3)*MASK_INT(4)).NE.0.) THEN
            !HSIMT PARABOLIC INTERPOLATION
                
              !COEFFICIENTS
              COF1 = 
     * 0.5*(VA_INT(4)-VA_INT(3))-0.5*(VA_INT(3)-VA_INT(2))
              COF2 = VA_INT(3)-VA_INT(2)
              COF3 = -1./6.*(VA_INT(4)-VA_INT(3))
     * -1./3.*(VA_INT(3)-VA_INT(2))+VA_INT(3)
